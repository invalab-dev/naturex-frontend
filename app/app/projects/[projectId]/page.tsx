...
import { getProjectById, getWidgetConfig, type Project, type Widget } from "@/lib/data-service"
...
export default function ProjectDashboardPage() {
  ...
  useEffect(() => {
    ...
    if (!preview && user?.role === "customer" && proj.orgId !== user.orgId) {
      toast({
        title: "접근 권한이 없습니다",
  ...
  return (
    <div className="min-h-screen bg-[#F5F7FB] p-8">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-[#111827] mb-2">{project.name}</h1>
        ...
        {widgets.length === 0 ? (
          <Card className="p-12 bg-white border-[#E5E7EB] text-center">
            ...
            <h3 className="text-lg font-semibold text-[#111827] mb-2">InvaLab에서 대시보드를 구성 중입니다</h3>
            <p className="text-sm text-[#6B7280] mb-6">곧 분석 결과가 제공될 예정입니다.</p>
          ...
          <div className="space-y-6">
            {widgets.map((widget) => (
              <Card key={widget.id} className="p-6 bg-white border-[#E5E7EB]">
                <h2 className="text-xl font-bold text-[#111827] mb-2">{widget.title}</h2>
                <p className="text-sm text-[#6B7280] mb-4">{widget.description}</p>
                <div className="bg-[#F5F7FB] rounded-lg p-8 text-center border border-[#E5E7EB]">
                  <p className="text-sm text-[#6B7280]">
                    위젯 컴포넌트: <span className="font-mono text-[#118DFF]">{widget.component}</span>
                  </p>
                  <p className="text-xs text-[#9CA3AF] mt-2">실제 대시보드 컴포넌트가 여기에 렌더링됩니다</p>
                </div>
              </Card>
            ))}
          </div>
