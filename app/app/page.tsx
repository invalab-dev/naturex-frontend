  ...
  useEffect(() => {
    ...
    const loadedProjects =
      user.role === "admin"
        ...
        : JSON.parse(localStorage.getItem("naturex_projects") || "[]").filter(
            (p: any) => p.orgId === user.orgId,
          );
      ...
      <div className="max-w-6xl mx-auto space-y-8">
        {user && (
          <div className="text-xs text-[#9CA3AF]">
            Session Debug: role={user.role}, userId={user.id}, orgId=
            {user.orgId || "N/A"}
          </div>
        )}
        ...
        <Card className="p-8 bg-white border-[#E5E7EB]">
          ...
          <p className="text-lg text-[#6B7280]">
            {orgName && (
              <span className="font-medium text-[#118DFF]">{orgName}</span>
            )}
            {orgName ? "의 " : ""}자연자산 프로젝트를 관리하고 분석 결과를
            확인하세요.
          </p>
        </Card>
