...
import { getWidgetCatalog, categoryLabels, type WidgetCatalogItem } from "@/lib/widget-catalog"
...
export default function WidgetCatalogPage() {
  const [widgets, setWidgets] = useState<WidgetCatalogItem[]>(getWidgetCatalog())
  const [searchQuery, setSearchQuery] = useState("")
  const [categoryFilter, setCategoryFilter] = useState<string>("all")
  const [statusFilter, setStatusFilter] = useState<string>("all")
  const [sortBy, setSortBy] = useState<"updated" | "name">("updated")
  const [selectedWidget, setSelectedWidget] = useState<WidgetCatalogItem | null>(null)
  ...
  const handleDeprecateWidget = (widget: WidgetCatalogItem) => {
    const updated = { ...widget, status: "deprecated" as const, updatedAt: new Date().toISOString() }
        ...
        <div className="bg-white border-b border-[#E5E7EB] px-8 py-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-sm text-[#6B7280] mt-1">전체 위젯 카탈로그 관리 · 프로젝트에서 사용할 위젯 정의</p>
            </div>
            <Button onClick={handleCreateWidget} className="bg-[#118DFF] hover:bg-[#0D6FCC] text-white gap-2">
          ...
          <div className="mb-6 space-y-4">
            ...
            <Card className="bg-white border-[#E5E7EB] p-4 rounded-xl shadow-sm">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-3xl font-bold text-[#111827]">{categoryCounts.all}</p>
                </div>
            ...
            {/* Category breakdown */}
            <div>
              <p className="text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-3">카테고리별 위젯</p>
              <div className="flex flex-wrap gap-2">
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm opacity-75">{categoryCounts.all}</span>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm opacity-75">{categoryCounts.common}</span>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm opacity-75">{categoryCounts.efficiency}</span>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm opacity-75">{categoryCounts.asset_value}</span>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm opacity-75">{categoryCounts.biodiversity}</span>
                  </div>
                </button>
              </div>
            </div>
            ...
            {/* Status breakdown */}
            <div>
              <p className="text-xs font-semibold text-[#6B7280] uppercase tracking-wider mb-3">상태별 위젯</p>
              <div className="flex flex-wrap gap-2">
                <button
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm opacity-75">{statusCounts.active}</span>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm opacity-75">{statusCounts.draft}</span>
                  </div>
                </button>
                ...
                >
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm opacity-75">{statusCounts.deprecated}</span>
                  </div>
                </button>
              </div>
            </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <Select value={sortBy} onValueChange={(v: "updated" | "name") => setSortBy(v)}>
              <SelectTrigger className="w-[140px] bg-white border-[#E5E7EB]">
                        ...
                        >
                          <td className="px-6 py-4">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                <div className="font-medium text-[#111827]">{widget.nameKr}</div>
                                <div className="text-xs text-[#6B7280]">{widget.nameEn}</div>
                              </div>
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="text-sm text-[#6B7280]">{categoryLabels[widget.category]}</div>
                          </td>
                          <td className="px-6 py-4">{getStatusBadge(widget.status)}</td>
                          <td className="px-6 py-4">
                            <div className="text-sm text-[#6B7280]">
                              {widget.defaultVisibility === "customer" ? (
                                ...
                                <span className="flex items-center gap-1 text-orange-700">관리자</span>
                              )}
                            </div>
                          </td>
                          <td className="px-6 py-4">
                            <div className="flex gap-1 flex-wrap">
                              {inputs.length === 0 ? (
                                ...
                                inputs.map((input) => (
                                  <span key={input} className="text-xs bg-[#F3F4F6] text-[#4B5563] px-2 py-1 rounded">
                                    {input}
                                  </span>
                                ))
                              )}
                            ...
                            <DropdownMenu>
                              ...
                              <DropdownMenuContent align="end" className="bg-white border-[#E5E7EB]">
                                ...
                                <DropdownMenuItem
                                  onClick={(e) => {
                                    e.stopPropagation()
                                    if (confirm(`"${widget.nameKr}" 위젯을 비활성화하시겠습니까?`)) {
