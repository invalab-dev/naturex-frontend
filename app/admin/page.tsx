...
export default function AdminHomePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      const efficiencyProjects = projects.filter(
        (p) => p.theme === "efficiency",
      );
      ...
      setServiceDelivery({
        efficiency: {
          ...
          delivering: efficiencyProjects.filter(
            (p) =>
              p.deliveryStage === "delivering" ||
              p.deliveryStage === "executing",
          ).length,
          pending: efficiencyProjects.filter(
            (p) => p.deliveryStage === "pending",
          ).length,
        },
        asset: {
          total: assetProjects.length,
          analyzing: assetProjects.filter(
            (p) => p.deliveryStage === "analyzing",
          ).length,
