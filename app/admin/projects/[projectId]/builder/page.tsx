...
export default function WidgetBuilderPage() {
  ...
  useEffect(() => {
    ...
    if (!config) {
      const themeWidgets = catalog.filter(
        (w) => w.category === proj.theme || w.category === 'common',
      );
      const initialWidgets: Widget[] = themeWidgets.map((w, index) => ({
        ...
        permission:
          w.defaultVisibility === 'customer' ? 'customer_view' : 'admin_only',
        order: index + 1,
  ...
  const handleUpdateWidget = (updates: Partial<Widget>) => {
    ...
    setWidgets((prev) =>
      prev.map((w) => (w.id === selectedWidget.id ? { ...w, ...updates } : w)),
    );
  ...
  const handleApplyDefaultTemplate = () => {
    ...
    if (confirm('기본 템플릿을 적용하시겠습니까? 현재 설정이 초기화됩니다.')) {
      const themeWidgets = catalogWidgets.filter(
        (w) => w.category === project.theme || w.category === 'common',
      );
      ...
      if (hasOverride) {
        ...
        if (widget && catalogWidget) {
          ...
          const hasChanges =
            ...
            widget.permission !==
              (catalogWidget.defaultVisibility === 'customer'
                ? 'customer_view'
                : 'admin_only');
          ...
          if (hasChanges) {
            saveProjectOverride({
              ...
              overrides: {
                ...
                defaultVisibility:
                  widget.permission === 'customer_view' ? 'customer' : 'admin',
              },
              updatedAt: new Date().toISOString(),
            });
          }
  ...
  const commonWidgets = availableCatalogWidgets.filter(
    (w) => w.category === 'common',
  );
  const themeWidgets = availableCatalogWidgets.filter(
    (w) => w.category !== 'common',
  );
    ...
    <div className="max-w-[1800px] mx-auto px-8 py-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-[#6B7280]">
              {project.name} · 고객사가 볼 대시보드 위젯을 구성하세요
            </p>
          </div>
      ...
      <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4 flex items-start gap-3">
        ...
        <div className="text-sm text-blue-800">
          ...
          <p>
            이 빌더는 <strong>위젯 설정</strong>에서 정의한 글로벌 위젯
            카탈로그를 사용합니다. 프로젝트별 설정(Override)을 활성화하면 이
            프로젝트만의 커스텀 설정을 적용할 수 있습니다.
          </p>
        </div>
      </div>
                  ...
                  <div className="space-y-2">
                    {themeWidgets.map((catalogWidget) => {
                      const widget = widgets.find(
                        (w) => w.id === catalogWidget.id,
                      );
                      const isSelected =
                        selectedWidget?.id === catalogWidget.id;
                      const hasOverride = projectOverrides.has(
                        catalogWidget.id,
                      );
                            ...
                            onCheckedChange={() => {
                              if (widget) {
                                ...
                                const newWidget: Widget = {
                                  ...
                                  enabled: true,
                                  permission:
                                    catalogWidget.defaultVisibility ===
                                    'customer'
                                      ? 'customer_view'
                                      : 'admin_only',
        ...
        <div className="col-span-6">
          <Card className="p-6 bg-white border-[#E5E7EB]">
            ...
            <p className="text-sm text-[#6B7280] mb-6">
              고객사가 보게 될 대시보드 ({enabledWidgets.length}개 위젯
              활성화됨)
            </p>
              ...
              <div className="space-y-4 max-h-[calc(100vh-280px)] overflow-y-auto pr-2">
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleUpdateWidget({ title: e.target.value })
                    }
                ...
                <div className="space-y-2">
                  ...
                  <Textarea
                    ...
                    onChange={(e) =>
                      handleUpdateWidget({ description: e.target.value })
                    }
                ...
                <div className="space-y-2">
                  ...
                  <Select
                    ...
                    onValueChange={(value: 'customer_view' | 'admin_only') =>
                      handleUpdateWidget({ permission: value })
                    }
                    disabled={!projectOverrides.has(selectedWidget.id)}
                  >
                  ...
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <Checkbox
                      ...
                      onCheckedChange={(checked) =>
                        handleUpdateWidget({ enabled: checked as boolean })
                      }
                    />
                  </div>
                  <p className="text-xs text-[#6B7280]">
                    {selectedWidget.enabled
                      ? '이 위젯이 대시보드에 표시됩니다'
                      : '이 위젯이 숨겨집니다'}
                  </p>
                </div>
