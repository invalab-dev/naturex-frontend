...
export default function AdminProjectsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    if (filterStage !== "all") out = out.filter((p) => p.deliveryStage === filterStage)
    if (searchQuery) {
      const q = searchQuery.toLowerCase()
      out = out.filter((p) => p.name.toLowerCase().includes(q) || p.location.toLowerCase().includes(q))
    }
  ...
  const submitStageChange = async () => {
    ...
    await updateProjectDeliveryStage(selectedProject.projectId, newStage, stageChangeMemo)
    await load()
    setIsStageEditOpen(false)
    setStageChangeMemo("")
    toast({ title: "단계 변경됨", description: `${selectedProject.name} → ${DELIVERY_STAGES[newStage].kr}` })
  }
    ...
    <div className="max-w-7xl mx-auto px-8 py-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <Link href="/admin/projects/new">
          <Button className="bg-[#118DFF] hover:bg-[#0D6FCC] text-white">프로젝트 생성</Button>
        </Link>
      </div>
        ...
        <div className="grid md:grid-cols-5 gap-4">
          ...
          <div className="space-y-2">
            ...
            <Select value={filterTheme} onValueChange={setFilterTheme}>
              ...
              <SelectContent className="bg-white border-[#E5E7EB]">
                <SelectItem value="all">전체</SelectItem>
                <SelectItem value="efficiency">{themeLabels.efficiency}</SelectItem>
                <SelectItem value="asset">{themeLabels.asset}</SelectItem>
                <SelectItem value="biodiversity">{themeLabels.biodiversity}</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div className="space-y-2">
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <div className="text-xs text-[#6B7280] mt-1">
                    {getOrgName(p.orgId)} · {themeLabels[p.theme]} · {DELIVERY_STAGES[p.deliveryStage as DeliveryStage]?.kr ?? p.deliveryStage}
                  </div>
                  <div className="text-xs text-[#6B7280] mt-1">{p.location}</div>
                </div>
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setNewStage((p.deliveryStage as DeliveryStage) || 'pending')
      ...
      {isStageEditOpen && selectedProject && (
        <div className="fixed inset-0 bg-black/40 flex items-center justify-center p-6 z-50">
          <Card className="w-full max-w-xl p-6 bg-white space-y-4">
            <div className="text-lg font-semibold">단계 변경: {selectedProject.name}</div>
            <div className="space-y-2">
              <Label>새 단계</Label>
            ...
            <div className="flex justify-end gap-2">
              <Button variant="outline" onClick={() => setIsStageEditOpen(false)}>
                취소
              </Button>
              <Button className="bg-[#118DFF] hover:bg-[#0D6FCC] text-white" onClick={submitStageChange}>
                저장
              </Button>
            </div>
          </Card>
