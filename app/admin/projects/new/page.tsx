...
import { ArrowLeft, FolderKanban, CheckCircle2, Leaf, TrendingUp, TreePine, Search, Map, Download, BarChart3 } from "lucide-react"
import Link from "next/link"
import { useRouter } from "next/navigation"
import { createProject, getOrganizations, type Organization, type ProjectTheme, type ResultConfig } from "@/lib/data-service"
...
export default function NewProjectPage() {
  ...
  const filteredOrgs = orgs.filter(
    (org) => org.name.toLowerCase().includes(orgSearch.toLowerCase()) || (org.industry || "").toLowerCase().includes(orgSearch.toLowerCase()),
  )
  ...
  const toggleMapType = (type: "geojson" | "tiles3d" | "laz") => {
    setResultConfig((prev) => ({
      ...
      map: {
        ...prev.map,
        types: prev.map.types.includes(type) ? prev.map.types.filter((t) => t !== type) : [...prev.map.types, type],
      },
    }))
  }
  ...
  const toggleTableType = (type: "table" | "bar" | "line" | "kpi") => {
    setResultConfig((prev) => ({
      ...
      tables: {
        ...prev.tables,
        types: prev.tables.types.includes(type) ? prev.tables.types.filter((t) => t !== type) : [...prev.tables.types, type],
      },
    }))
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!formData.orgId || !formData.theme || !formData.name || !formData.location) return
  ...
  const themeOptions: Array<{ value: ProjectTheme; label: string; icon: any; description: string }> = [
    { value: "efficiency", label: "운영비 절감", icon: TrendingUp, description: "유지관리 효율화, 에너지 절감, 비용 최적화" },
    { value: "asset", label: "자산 가치 향상", icon: Leaf, description: "ESG 평가, 자산 가치 정량화, 포트폴리오 우선순위" },
    { value: "biodiversity", label: "생물다양성 프로젝트", icon: TreePine, description: "생태복원, 서식지 분석, TNFD 대응" },
  ]
        ...
        <div className="mb-8">
          ...
          <div className="flex items-center gap-3 mb-2">
            ...
            <div>
              ...
              <p className="text-sm text-[#6B7280]">프로젝트 정보와 결과 제공 유형을 설정합니다.</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-5 gap-6">
                        ...
                        {filteredOrgs.length === 0 ? (
                          ...
                          filteredOrgs.map((org) => (
                            ...
                            >
                              ...
                              {formData.orgId === org.orgId && <CheckCircle2 className="w-5 h-5 text-[#118DFF]" />}
                            </button>
                          ))
                        )}
                    ...
                    {themeOptions.map((t) => (
                      <button
                        ...
                        className={`p-4 rounded-lg border text-left ${
                          formData.theme === t.value ? "border-[#118DFF] bg-[#118DFF]/5" : "border-[#E5E7EB] bg-white"
                        }`}
                      >
                        <div className="font-semibold text-[#111827]">{t.label}</div>
                        <div className="text-xs text-[#6B7280] mt-1">{t.description}</div>
                      </button>
                    ))}
                  </div>
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            <Card className="bg-white border-[#E5E7EB] p-6 space-y-4">
              <div>
                <h2 className="text-lg font-semibold text-[#111827]">결과 제공 설정</h2>
                <p className="text-sm text-[#6B7280]">프로젝트 납품 유형을 선택하세요.</p>
              </div>
              ...
              <div className="space-y-3">
                <div className="flex items-start gap-3 p-4 rounded-lg border border-[#E5E7EB]">
                  <Checkbox
                    ...
                    onCheckedChange={(v) => setResultConfig((p) => ({ ...p, map: { ...p.map, enabled: !!v } }))}
                    className="mt-1"
                  />
                  <div className="flex-1">
                    ...
                    <div className="text-xs text-[#6B7280] mt-1">GeoJSON / 3D Tiles / LAZ</div>
                    {resultConfig.map.enabled && (
                      <div className="mt-3 space-y-2">
                        {(["geojson", "tiles3d", "laz"] as const).map((t) => (
                          <label key={t} className="flex items-center gap-2 text-sm">
                            <Checkbox checked={resultConfig.map.types.includes(t)} onCheckedChange={() => toggleMapType(t)} />
                            {t}
                          </label>
                        ))}
                      </div>
                ...
                <div className="flex items-start gap-3 p-4 rounded-lg border border-[#E5E7EB]">
                  <Checkbox
                    ...
                    onCheckedChange={(v) => setResultConfig((p) => ({ ...p, downloads: { enabled: !!v } }))}
                    className="mt-1"
                  />
                  <div className="flex-1">
                    <div className="flex items-center gap-2 font-medium">
                    ...
                    <div className="text-xs text-[#6B7280] mt-1">HWP/XLSX/PDF</div>
                  </div>
                </div>
                ...
                <div className="flex items-start gap-3 p-4 rounded-lg border border-[#E5E7EB]">
                  <Checkbox
                    ...
                    onCheckedChange={(v) => setResultConfig((p) => ({ ...p, tables: { ...p.tables, enabled: !!v } }))}
                    className="mt-1"
                  />
                  <div className="flex-1">
                    <div className="flex items-center gap-2 font-medium">
                    ...
                    <div className="text-xs text-[#6B7280] mt-1">table/bar/line/kpi</div>
                    {resultConfig.tables.enabled && (
                      <div className="mt-3 space-y-2">
                        {(["table", "bar", "line", "kpi"] as const).map((t) => (
                          <label key={t} className="flex items-center gap-2 text-sm">
                            <Checkbox checked={resultConfig.tables.types.includes(t)} onCheckedChange={() => toggleTableType(t)} />
                            {t}
                          </label>
                        ))}
                      </div>
