...
export function WidgetModalEditor({ widget, isOpen, onClose, onSave, onDelete }: WidgetModalEditorProps) {
  ...
  const [selectedVizPreview, setSelectedVizPreview] = useState<string | null>(null)
  ...
  const toggleServiceTheme = (theme: string) => {
    setServiceThemes((prev) => (prev.includes(theme) ? prev.filter((t) => t !== theme) : [...prev, theme]))
    setHasUnsavedChanges(true)
  }

  const toggleUsageStage = (stage: string) => {
    setUsageStages((prev) => (prev.includes(stage) ? prev.filter((s) => s !== stage) : [...prev, stage]))
    setHasUnsavedChanges(true)
  }
  ...
  const updateExtendedMetric = (index: number, updates: Partial<ExtendedMetric>) => {
  ...
  const removeReference = (metricIndex: number, refIndex: number) => {
    const newMetrics = [...extendedMetrics]
    newMetrics[metricIndex].references = newMetrics[metricIndex].references!.filter((_, i) => i !== refIndex)
    setExtendedMetrics(newMetrics)
    setHasUnsavedChanges(true)
  }

  const updateReference = (metricIndex: number, refIndex: number, field: "title" | "source", value: string) => {
    const newMetrics = [...extendedMetrics]
  ...
  const visualizationTypes = [
    { type: "heatmap", label: "지도 Heatmap", icon: Map, desc: "밀도 기반 히트맵 시각화" },
    { type: "choropleth", label: "Choropleth", icon: Grid3X3, desc: "영역별 색상 등급 지도" },
    { type: "point_polygon", label: "Point / Polygon Map", icon: Map, desc: "포인트 및 폴리곤 지도" },
    { type: "bar_chart", label: "Bar Chart", icon: BarChart3, desc: "막대형 비교 차트" },
    { type: "line_chart", label: "Line Chart", icon: LineChart, desc: "시계열 추이 차트" },
    { type: "gauge", label: "Gauge / Score Card", icon: Gauge, desc: "게이지 또는 스코어 카드" },
  ]
  ...
  const exportFormatOptions = [
    ...
    { key: "docx", label: "Word (.docx)", desc: "Microsoft Word 문서", group: "reports" },
      ...
      case "heatmap":
        return (
          <div className="bg-gradient-to-br from-green-200 via-yellow-200 to-red-300 rounded-lg h-48 flex items-center justify-center relative overflow-hidden">
            ...
            <span className="text-sm text-gray-700 bg-white/80 px-3 py-1 rounded">Heatmap Preview</span>
          </div>
        )
      case "choropleth":
        return (
        ...
        return (
          <div className="bg-white rounded-lg h-48 flex flex-col items-center justify-center border border-[#E5E7EB] p-4">
            ...
            <div className="w-32 h-2 bg-gray-200 rounded-full mt-3 overflow-hidden">
              <div className="h-full bg-[#118DFF] rounded-full" style={{ width: "78.5%" }} />
            </div>
          </div>
        )
      default:
            ...
            <div className="flex items-start justify-between mb-3">
              <div className="flex-1">
                <Input
                  ...
                  placeholder="위젯 이름"
                />
                <p className="text-sm text-[#6B7280] mt-1">분석 로직 · 시각화 · 서비스 정의</p>
              </div>
            ...
            <Tabs defaultValue="overview" className="w-full">
              ...
              <TabsContent value="overview" className="p-6 space-y-6">
                ...
                <div className="bg-white border border-[#E5E7EB] rounded-lg p-5 space-y-4">
                  <h3 className="font-semibold text-[#111827] text-base">기본 정보</h3>
                ...
                <div className="bg-white border border-[#E5E7EB] rounded-lg p-5 space-y-4">
                  <div>
                    <h3 className="font-semibold text-[#111827] text-base">서비스 테마 적용</h3>
                    <p className="text-sm text-[#6B7280] mt-1">이 위젯이 사용될 수 있는 서비스 테마를 선택하세요 (복수 선택 가능)</p>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    >
                      <TrendingUp className={`w-6 h-6 mb-2 ${serviceThemes.includes("efficiency") ? "text-white" : "text-[#118DFF]"}`} />
                      <div className={`font-medium ${serviceThemes.includes("efficiency") ? "text-white" : "text-[#111827]"}`}>
                        운영비 절감
                      </div>
                      <div className={`text-xs mt-1 ${serviceThemes.includes("efficiency") ? "text-white/80" : "text-[#6B7280]"}`}>
                        Efficiency
                      </div>
                    </button>
                    ...
                    >
                      <Leaf className={`w-6 h-6 mb-2 ${serviceThemes.includes("asset_value") ? "text-white" : "text-[#22C55E]"}`} />
                      <div className={`font-medium ${serviceThemes.includes("asset_value") ? "text-white" : "text-[#111827]"}`}>
                        자산 가치 향상
                      </div>
                      <div className={`text-xs mt-1 ${serviceThemes.includes("asset_value") ? "text-white/80" : "text-[#6B7280]"}`}>
                        Asset Value
                      </div>
                    </button>
                    ...
                    >
                      <Bug className={`w-6 h-6 mb-2 ${serviceThemes.includes("biodiversity") ? "text-white" : "text-[#8B5CF6]"}`} />
                      <div className={`font-medium ${serviceThemes.includes("biodiversity") ? "text-white" : "text-[#111827]"}`}>
                        생물다양성 프로젝트
                      </div>
                      <div className={`text-xs mt-1 ${serviceThemes.includes("biodiversity") ? "text-white/80" : "text-[#6B7280]"}`}>
                        Biodiversity
                      </div>
                    </button>
                  </div>
                ...
                <div className="bg-white border border-[#E5E7EB] rounded-lg p-5 space-y-4">
                  <div>
                    <h3 className="font-semibold text-[#111827] text-base">사용 단계</h3>
                    <p className="text-sm text-[#6B7280] mt-1">이 위젯이 사용되는 서비스 단계를 선택하세요</p>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    ...
                    >
                      <Clipboard className={`w-6 h-6 mb-2 ${usageStages.includes("planning") ? "text-white" : "text-[#118DFF]"}`} />
                      <div className={`font-medium ${usageStages.includes("planning") ? "text-white" : "text-[#111827]"}`}>
                        계획
                      </div>
                      <div className={`text-xs mt-1 ${usageStages.includes("planning") ? "text-white/80" : "text-[#6B7280]"}`}>
                        Planning
                      </div>
                    </button>
                    ...
                    >
                      <BarChart3 className={`w-6 h-6 mb-2 ${usageStages.includes("analysis") ? "text-white" : "text-[#118DFF]"}`} />
                      <div className={`font-medium ${usageStages.includes("analysis") ? "text-white" : "text-[#111827]"}`}>
                        분석
                      </div>
                      <div className={`text-xs mt-1 ${usageStages.includes("analysis") ? "text-white/80" : "text-[#6B7280]"}`}>
                        Analysis
                      </div>
                    </button>
                    ...
                    >
                      <Settings className={`w-6 h-6 mb-2 ${usageStages.includes("management") ? "text-white" : "text-[#118DFF]"}`} />
                      <div className={`font-medium ${usageStages.includes("management") ? "text-white" : "text-[#111827]"}`}>
                        관리
                      </div>
                      <div className={`text-xs mt-1 ${usageStages.includes("management") ? "text-white/80" : "text-[#6B7280]"}`}>
                        Management
                      </div>
                    </button>
                  </div>
              ...
              <TabsContent value="inputs" className="p-6 space-y-6">
                <div className="bg-white border border-[#E5E7EB] rounded-lg p-5 space-y-4">
                  <div>
                    <h3 className="font-semibold text-[#111827] text-base">필수 데이터 소스</h3>
                    <p className="text-sm text-[#6B7280] mt-1">이 위젯 분석에 필요한 데이터 소스를 선택하세요</p>
                  </div>
                  ...
                  <div className="space-y-3">
                    {dataSources.map(({ key, label, desc }) => {
                      ...
                      const isChecked = isCustom
                        ...
                        : editedWidget.requiredInputs[key as keyof typeof editedWidget.requiredInputs]

                      return (
                        <div key={key}>
                          <div
                            className={`flex items-center justify-between p-4 bg-[#F9FAFB] border rounded-lg transition-colors ${
                              isChecked ? "border-[#118DFF] bg-[#118DFF]/5" : "border-[#E5E7EB] hover:border-[#118DFF]"
                            }`}
                          >
                            <div className="flex-1">
                          ...
                          {isCustom && isChecked && (
                            <div className="mt-3 ml-4 p-4 bg-[#F9FAFB] border border-[#E5E7EB] rounded-lg">
                              <Label htmlFor="customDataDesc" className="text-sm font-medium text-[#111827]">
                                기타 데이터 설명
                              </Label>
                              <Textarea
                                id="customDataDesc"
              ...
              <TabsContent value="metrics" className="p-6 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-semibold text-[#111827] text-base">지표 목록</h3>
                    ...
                    {extendedMetrics.map((metric, index) => (
                      <div key={index} className="bg-white border border-[#E5E7EB] rounded-lg overflow-hidden">
                        ...
                        <div className="flex items-center justify-between px-5 py-4 bg-[#F9FAFB] border-b border-[#E5E7EB]">
                          <div className="flex items-center gap-3">
                            <span className="text-sm font-semibold text-[#118DFF]">지표 #{index + 1}</span>
                            <span className="text-sm text-[#6B7280]">{metric.labelKr || "이름 없음"}</span>
                          </div>
                          <div className="flex gap-1">
                            ...
                            <Button
                              ...
                              className="h-8 w-8 p-0"
                              onClick={() => {
                                if (index === extendedMetrics.length - 1) return
                                const newMetrics = [...extendedMetrics]
                            ...
                            <Button
                              ...
                              onClick={() => {
                                setExtendedMetrics(extendedMetrics.filter((_, i) => i !== index))
                                setHasUnsavedChanges(true)
                              }}
                            >
                        ...
                        <div className="p-5 space-y-5">
                          ...
                          <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                              <Label className="text-sm font-medium">지표 이름 (한글)</Label>
                              <Input
                                placeholder="위험도 점수"
                                value={metric.labelKr}
                                onChange={(e) => updateExtendedMetric(index, { labelKr: e.target.value })}
                                className="bg-white border-[#E5E7EB]"
                              />
                            </div>
                            <div className="space-y-2">
                              <Label className="text-sm font-medium">Metric Name (English)</Label>
                              <Input
                                placeholder="Risk Score"
                                value={metric.labelEn}
                                onChange={(e) => updateExtendedMetric(index, { labelEn: e.target.value })}
                                className="bg-white border-[#E5E7EB]"
                              />
                            </div>
                          </div>
                          ...
                          <div className="grid grid-cols-3 gap-4">
                            <div className="space-y-2">
                              <Label className="text-sm font-medium">Key (API 필드명)</Label>
                              <Input
                                placeholder="risk_score"
                                value={metric.key}
                                onChange={(e) => updateExtendedMetric(index, { key: e.target.value })}
                            ...
                            <div className="space-y-2">
                              ...
                              <Input
                                placeholder="%"
                                value={metric.unit || ""}
                                onChange={(e) => updateExtendedMetric(index, { unit: e.target.value || null })}
                                className="bg-white border-[#E5E7EB]"
                              />
                            </div>
                            <div className="space-y-2">
                              ...
                              <Select
                                value={metric.format}
                                onValueChange={(value: any) => updateExtendedMetric(index, { format: value })}
                              >
                          ...
                          {/* 계산 개요 설명 */}
                          <div className="space-y-2">
                            <Label className="text-sm font-medium">계산 개요 설명</Label>
                            <Textarea
                              placeholder="이 지표가 어떻게 계산되는지 간략히 설명하세요"
                              value={metric.descriptionKr || ""}
                              onChange={(e) => updateExtendedMetric(index, { descriptionKr: e.target.value })}
                          ...
                          <div className="space-y-2 bg-[#F9FAFB] p-4 rounded-lg">
                            <Label className="text-sm font-medium text-[#111827]">지표 의미 설명</Label>
                            <p className="text-xs text-[#6B7280] mb-2">이 지표가 무엇을 나타내며, 의사결정에 어떻게 활용되는지 설명하세요</p>
                            <Textarea
                              ...
                              onChange={(e) => updateExtendedMetric(index, { meaningKr: e.target.value })}
                          ...
                          {/* 해석 및 원인 설명 */}
                          <div className="space-y-2 bg-[#F9FAFB] p-4 rounded-lg">
                            <Label className="text-sm font-medium text-[#111827]">해석 및 원인 설명</Label>
                            <p className="text-xs text-[#6B7280] mb-2">높은/낮은 값의 의미와 가능한 원인을 설명하세요 (리포트에 활용됨)</p>
                            <Textarea
                              ...
                              onChange={(e) => updateExtendedMetric(index, { interpretationKr: e.target.value })}
                              className="bg-white border-[#E5E7EB] min-h-[100px]"
                            />
                          </div>
                          ...
                          <div className="space-y-3 bg-[#F9FAFB] p-4 rounded-lg">
                            <div className="flex items-center justify-between">
                              <div>
                                <Label className="text-sm font-medium text-[#111827]">참고 문헌</Label>
                                <p className="text-xs text-[#6B7280] mt-0.5">관련 논문, 표준, 가이드라인 등</p>
                              </div>
                              <Button
                                size="sm"
                                variant="outline"
                            ...
                            {metric.references && metric.references.length > 0 ? (
                              <div className="space-y-2">
                                {metric.references.map((ref, refIndex) => (
                                  <div key={refIndex} className="flex gap-2 items-start bg-white p-3 rounded-lg border border-[#E5E7EB]">
                                    <div className="flex-1 space-y-2">
                                      <Input
                                        placeholder="문헌 제목"
                                        value={ref.title}
                                        onChange={(e) => updateReference(index, refIndex, "title", e.target.value)}
                                      ...
                                      <div className="flex items-center gap-2">
                                        ...
                                        <Input
                                          ...
                                          value={ref.source}
                                          onChange={(e) => updateReference(index, refIndex, "source", e.target.value)}
                                          className="bg-white border-[#E5E7EB] text-sm"
                                        />
                                      </div>
                                    </div>
              ...
              <TabsContent value="visuals" className="p-6 space-y-6">
                <div className="bg-white border border-[#E5E7EB] rounded-lg p-5 space-y-4">
                  <div>
                    <h3 className="font-semibold text-[#111827] text-base">지원 시각화 타입</h3>
                    <p className="text-sm text-[#6B7280] mt-1">이 위젯이 지원하는 차트 및 맵 표현 방식을 선택하세요</p>
                  </div>

                  <div className="grid grid-cols-3 gap-3">
                    {visualizationTypes.map(({ type, label, icon: Icon, desc }) => {
                      const viz = editedWidget.visualizations.find((v) => v.type === type)
                      ...
                      return (
                        <button
                          key={type}
                          onClick={() => {
                            const newViz = editedWidget.visualizations.map((v) =>
                              v.type === type ? { ...v, enabled: !v.enabled } : v
                            )
                            // Add if not exists
                            if (!editedWidget.visualizations.find((v) => v.type === type)) {
                              newViz.push({ type: type as any, enabled: true })
                            }
                        ...
                        >
                          <Icon className={`w-6 h-6 mb-2 ${isEnabled ? "text-[#118DFF]" : "text-[#6B7280]"}`} />
                          <div className={`font-medium text-sm ${isEnabled ? "text-[#118DFF]" : "text-[#111827]"}`}>
                            {label}
                          </div>
                          <div className="text-xs text-[#6B7280] mt-1">{desc}</div>
                        </button>
                      )
                    })}
                  </div>
                ...
                <div className="bg-white border border-[#E5E7EB] rounded-lg p-5 space-y-4">
                  <div className="flex items-center justify-between">
                    <div>
                      <h3 className="font-semibold text-[#111827] text-base">고객 화면 예시</h3>
                      <p className="text-sm text-[#6B7280] mt-1">선택한 시각화가 고객에게 어떻게 보이는지 미리보기</p>
                    </div>
                    <Select value={selectedVizPreview || ""} onValueChange={setSelectedVizPreview}>
                  ...
                  <div className="border border-dashed border-[#E5E7EB] rounded-lg p-4">
                    ...
                    {selectedVizPreview ? (
                      ...
                      <div className="bg-gray-50 rounded-lg h-48 flex items-center justify-center text-[#9CA3AF]">
                        <p className="text-sm">시각화 타입을 선택하면 미리보기가 표시됩니다</p>
                      </div>
                    )}
                  </div>
                </div>
              ...
              <TabsContent value="exports" className="p-6 space-y-6">
                <div className="bg-white border border-[#E5E7EB] rounded-lg p-5 space-y-4">
                  <div>
                    <h3 className="font-semibold text-[#111827] text-base">다운로드 포함</h3>
                    <p className="text-sm text-[#6B7280] mt-1">고객이 다운로드할 수 있는 파일 형식을 선택하세요</p>
                  </div>

                  <div className="space-y-4">
                    <div>
                      <Label className="text-sm font-medium text-[#6B7280] uppercase tracking-wider">리포트</Label>
                      <div className="grid grid-cols-3 gap-3 mt-2">
                        {exportFormatOptions
                          ...
                          .map(({ key, label, desc }) => (
                            ...
                            >
                              <FileText className={`w-5 h-5 mb-2 ${exportFormats.includes(key) ? "text-[#118DFF]" : "text-[#6B7280]"}`} />
                              <div className={`font-medium text-sm ${exportFormats.includes(key) ? "text-[#118DFF]" : "text-[#111827]"}`}>
                                {label}
                              </div>
                              <div className="text-xs text-[#6B7280] mt-1">{desc}</div>
                            </button>
                    ...
                    <div>
                      <Label className="text-sm font-medium text-[#6B7280] uppercase tracking-wider">데이터</Label>
                      <div className="grid grid-cols-3 gap-3 mt-2">
                        {exportFormatOptions
                          ...
                          .map(({ key, label, desc }) => (
                            ...
                            >
                              <FileSpreadsheet className={`w-5 h-5 mb-2 ${exportFormats.includes(key) ? "text-[#118DFF]" : "text-[#6B7280]"}`} />
                              <div className={`font-medium text-sm ${exportFormats.includes(key) ? "text-[#118DFF]" : "text-[#111827]"}`}>
                                {label}
                              </div>
                              <div className="text-xs text-[#6B7280] mt-1">{desc}</div>
