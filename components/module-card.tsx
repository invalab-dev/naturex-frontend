...
export function ModuleCard({ title, description, usp, icon: Icon, href, onEnter }: ModuleCardProps) {
  const handleClick = () => {
    if (onEnter) onEnter()
  }
      ...
      <CardContent className="flex-1">
        {usp && (
          <div className="mb-4 p-3 rounded-lg bg-primary/5 border border-primary/20">
            <p className="text-sm font-semibold text-foreground leading-relaxed line-clamp-2">{usp}</p>
          </div>
        )}
