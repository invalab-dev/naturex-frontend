...
export function InteractiveMap({ onTreeSelect }: InteractiveMapProps) {
  ...
  const latLngToXY = (lat: number, lng: number) => {
    ...
    const y = mapHeight - ((lat - (centerLat - latRange / 2)) / latRange) * mapHeight
    return { x, y }
  }
    ...
    <div className="w-full h-full bg-gray-100 dark:bg-gray-900 relative overflow-hidden">
      ...
      >
        ...
        {Array.from({ length: 20 }).map((_, i) => {
          ...
          return <rect key={`building-${i}`} x={x} y={y} width={100} height={80} fill="#cbd5e1" opacity="0.4" rx="2" />
        })}
        ...
        {trees.map((tree) => {
          ...
          return (
            ...
            >
              ...
              {/* Marker outer ring */}
              <circle cx={x} cy={y} r={markerSize} fill="white" stroke={markerColor} strokeWidth={isSelected ? 3 : 2} />

              {/* Marker inner circle */}
              <circle cx={x} cy={y} r={markerSize - 4} fill={markerColor} opacity={0.9} />

              {/* Selection pulse animation */}
              {isSelected && (
                <circle cx={x} cy={y} r={markerSize + 4} fill="none" stroke={markerColor} strokeWidth="2" opacity="0.5">
                  <animate
                    attributeName="r"
                    from={markerSize}
                    to={markerSize + 8}
              ...
              {isHovered && (
                <g>
                  ...
                  />
                  <text x={x + 30} y={y - 15} fontSize="11" fontWeight="600" fill="#111827">
                    {tree.id}
                  </text>
                  <text x={x + 30} y={y - 3} fontSize="9" fill="#6b7280">
                    {tree.name}
                  </text>
                  <text x={x + 30} y={y + 9} fontSize="9" fill={markerColor} fontWeight="500">
                    {tree.riskLevel} Risk
                  </text>
                </g>
              )}
