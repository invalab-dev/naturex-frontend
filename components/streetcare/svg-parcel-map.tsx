...
export function SvgParcelMap({
  ...
  return (
    <div className={`relative w-full h-full rounded-xl overflow-hidden ${getBackground()}`}>
      {/* Layer Switcher */}
      ...
      <svg viewBox="0 0 1000 600" className="w-full h-full">
        <defs>
          <pattern id="grid" width="40" height="40" patternUnits="userSpaceOnUse">
            <path d="M 40 0 L 0 0 0 40" fill="none" stroke={getGridColor()} strokeWidth="0.5" opacity="0.4" />
          </pattern>
          <filter id="glow">
        ...
        {/* Parcels */}
        {parcels.map((parcel) => {
          const pointsAttr = parcel.points.map((p) => `${p.x},${p.y}`).join(" ")
          ...
          return (
            <g key={parcel.id}>
              ...
              <polygon
                ...
                stroke={isSelected ? "#ffffff" : isHovered ? "#ffffff" : "#0f172a"}
              ...
              />
              <text
                x={parcel.points.reduce((sum, p) => sum + p.x, 0) / parcel.points.length}
                y={parcel.points.reduce((sum, p) => sum + p.y, 0) / parcel.points.length}
                textAnchor="middle"
                fill="white"
                fontSize="14"
                fontWeight="bold"
            ...
            <g key={tree.id}>
              ...
              {isHovered && (
                <g>
                  ...
                  />
                  <text x={tree.x + 18} y={tree.y - 12} fill="white" fontSize="11" fontWeight="bold">
                    {tree.id}
                  </text>
                  <text x={tree.x + 18} y={tree.y + 2} fill="white" fontSize="10">
                    {tree.species}
                  </text>
                  <text x={tree.x + 18} y={tree.y + 16} fill={color} fontSize="10" fontWeight="bold">
                    {tree.risk} Risk
                  </text>
                </g>
              )}
