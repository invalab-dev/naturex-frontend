...
import { SharedSidebar, SidebarMenuItem } from "@/components/sidebar/shared-sidebar"
...
export function GlobalSidebar() {
  ...
  const navigationContent = (
    <>
      <Link href="/app">
        <SidebarMenuItem active={isActive("/app")} icon={<Home className="w-5 h-5" />} label="홈" />
      </Link>

      <Link href="/app/projects">
        <SidebarMenuItem active={isActive("/app/projects")} icon={<FolderKanban className="w-5 h-5" />} label="프로젝트" />
      </Link>

      <Link href="/app/settings">
        <SidebarMenuItem active={isActive("/app/settings")} icon={<Settings className="w-5 h-5" />} label="설정" />
      </Link>
    </>
  )
  ...
  return <SharedSidebar header={headerContent} navigation={navigationContent} footer={footerContent} />
}
