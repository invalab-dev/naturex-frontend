...
export function ForestInventoryPanel({ plotId }: ForestInventoryPanelProps) {
  const data = plotId ? plotData[plotId as keyof typeof plotData] : plotData["FP-001"]
      ...
      <CardHeader className="border-b border-border pb-4">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground mt-1">Forest Inventory Data</p>
          </div>
          <Badge variant={getHealthBadge(data.healthStatus)}>{data.healthStatus.toUpperCase()}</Badge>
        </div>
      </CardHeader>
      ...
      <CardContent className="flex-1 overflow-auto p-6">
        ...
        <div className="grid grid-cols-2 gap-4 mb-6">
          <div className="border border-border rounded-lg p-3">
            ...
            <div className="text-2xl font-bold">{data.treeCount.toLocaleString()}</div>
            <div className="text-xs text-muted-foreground mt-1">{data.area}</div>
          </div>
          ...
          <div className="border border-border rounded-lg p-3">
            ...
            <div className="text-xs text-muted-foreground mt-1">DBH: {data.avgDbh}</div>
          </div>

          <div className="border border-border rounded-lg p-3">
            <div className="flex items-center gap-2 mb-2">
              <Activity size={16} className="text-primary" />
              <span className="text-xs text-muted-foreground">Timber Volume</span>
            </div>
            <div className="text-2xl font-bold">{data.timberVolume}</div>
            <div className="text-xs text-muted-foreground mt-1">Total standing volume</div>
          </div>
          ...
          <div className="border border-border rounded-lg p-3">
            ...
            <div className="text-xs text-muted-foreground mt-1">Shannon Index</div>
          </div>
        </div>

        {/* Species Info */}
        <div className="mb-6 p-3 bg-muted/30 rounded-lg">
          <div className="text-xs text-muted-foreground mb-1">Dominant Species</div>
          <div className="font-medium">{data.dominantSpecies}</div>
          <div className="text-xs text-muted-foreground mt-2">Last Survey: {data.lastSurvey}</div>
        </div>
